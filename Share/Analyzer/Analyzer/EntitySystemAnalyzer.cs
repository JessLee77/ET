...
namespace ET.Analyzer
{
    ...
    public class EntitySystemAnalyzer : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
                ImmutableArray.Create(EntitySystemAnalyzerRule.Rule, EntitySystemMethodNeedSystemOfAttrAnalyzerRule.Rule);
        ...
        private static ImmutableArray<ETSystemData> SupportedETSystemDatas => ImmutableArray.Create(new ETSystemData(
                Definition.EntitySystemOfAttribute, Definition.EntitySystemAttribute, Definition.EntityType, Definition.EntitySystemAttributeMetaName,
            ...
            new ETSystemData(Definition.LSEntitySystemOfAttribute, Definition.LSEntitySystemAttribute, Definition.LSEntityType,
                Definition.LSEntitySystemAttributeMetaName,
                new SystemMethodData(Definition.IAwakeInterface, Definition.AwakeMethod),
        ...
        private class ETSystemData
        {
            ...
            public ETSystemData(string systemOfAttribute, string systemAttributeShowName, string entityTypeName, string systemAttributeMetaName,
            params SystemMethodData[] systemMethods)
            {
        ...
        private void AnalyzeETSystem(SymbolAnalysisContext context, ETSystemData etSystemData,
                    ...
                    if (interfacetypeSymbol.IsInterface(systemMethodData.InterfaceName))
                    {
                        ...
                        else
                        {
                            if (interfacetypeSymbol.IsInterface(Definition.IGetComponentInterface))
                            {
                                ...
                                {
                                    AddProperty(ref builder, systemMethodData.MethodName,
                                        $"{entityTypeSymbol}/{etSystemData.SystemAttributeShowName}/System.Type");
                                }
                            }
        ...
        private void ReportNeedGenerateSystem(SymbolAnalysisContext context, INamedTypeSymbol namedTypeSymbol,
        ref ImmutableDictionary<string, string?>.Builder? builder)
        {
            if (builder == null)
            {
        ...
        private void ReportNeedSystemOfAttr(SymbolAnalysisContext context, IMethodSymbol methodSymbol, ETSystemData etSystemData)
        {
            foreach (SyntaxReference? reference in methodSymbol.DeclaringSyntaxReferences)
            {
                ...
                {
                    Diagnostic diagnostic = Diagnostic.Create(EntitySystemMethodNeedSystemOfAttrAnalyzerRule.Rule,
